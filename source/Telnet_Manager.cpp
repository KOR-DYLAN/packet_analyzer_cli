#include "../header/Telnet_Manager.h"
#include <iostream>
#include <cstring>
using namespace std;
...
string CTelnet_Manager::GetCmdName(uint8_t CmdCode) const {
    ...
    case TelnetCmd::End_subNeg:
        return string("[ End subNeg ] => End of option subnegotiation command.");
    case TelnetCmd::No_Operation:      
        return string("[ No Operation ] => No operation command.");
    case TelnetCmd::Data_Mark:     
        return string("[ Data Mark ] => End of urgent data stream. ");
    case TelnetCmd::Break:      
        return string("[ Break ] => Operator pressed the Break key or the Attention key.");
    case TelnetCmd::Int_process:     
        return string("[ Int process ] => Interrupt current process."); 
    case TelnetCmd::Abort_output:      
        return string("[ Abort output ] => Cancel output from current process.");
    case TelnetCmd::You_there:    
        return string("[ You there? ] => Request acknowledgment.");
    case TelnetCmd::Erase_char:     
        return string("[ Erase char ] => Request that operator erase the previous character.");
    case TelnetCmd::Erase_line:     
        return string("[ Erase line ] => Request that operator erase the previous line.");
    case TelnetCmd::Go_ahead:    
        return string("[ Go ahead! ] => End of input for half-duplex connections.");
    case TelnetCmd::SubNegotiate:      
        return string("[ SubNegotiate ] => Begin option subnegotiation.");
    case TelnetCmd::Will_Use:     
        return string("[ Will Use ] => Agreement to use the specified option.");
    case TelnetCmd::Wont_Use:      
        return string("[ Won't Use ] => Reject the proposed option.");
    case TelnetCmd::Start_use:     
        return string("[ Start use ] => Request to start using specified option.");
    case TelnetCmd::Stop_Use:   
        return string("[ Stop Use ] => Demand to stop using specified option."); 
    case TelnetCmd::IAC:      
        return string("[ IAC ] => Interpret as command.");
    default:
        return string("[ Unknown ] => Maybe new environment.");
    }
}
...
string CTelnet_Manager::GetOptName(uint8_t OptCode) const {
    ...
    case TelnetOpt::Binary_Xmit: 
        return string("[ Binary Xmit ] => Allows transmission of binary data.");
    case TelnetOpt::Echo_Data: 
        return string("[ Echo Data ] => Causes server to echo back all keystrokes.");
    case TelnetOpt::Reconnect:      
        return string("[ Reconnect ] => Reconnects to another TELNET host.");
    case TelnetOpt::Suppress_GA:     
        return string("[ Suppress GA ] => Disables Go Ahead! command.");
    case TelnetOpt::Message_Sz:     
        return string("[ Message Sz ] => Conveys approximate message size.");
    case TelnetOpt::Opt_Status:      
        return string("[ Opt Status ] => Lists status of options.");
    case TelnetOpt::Timing_Mark:    
        return string("[ Timing Mark ] => Marks a data stream position for reference. ");
    case TelnetOpt::RC_XmtEcho:     
        return string("[ RC XmtEcho ] => Allows remote control of terminal printers.");
    case TelnetOpt::Line_Width:      
        return string("[ Line Width ] => Sets output line width.");
    case TelnetOpt::Page_Length:    
        return string("[ Page Length ] => Sets page length in lines.");
    case TelnetOpt::CR_Use:          
        return string("[ CR Use ] => Determines handling of carriage returns.");
    case TelnetOpt::Horiz_Tabs:      
        return string("[ Horiz Tabs ] => Sets horizontal tabs.");
    case TelnetOpt::Hor_Tab_Use:     
        return string("[ Hor Tab Use ] => Determines handling of horizontal tabs.");
    case TelnetOpt::FF_Use:          
        return string("[ FF Use ] => Determines handling of form feeds.");
    case TelnetOpt::Vert_Tabs:      
        return string("[ Vert Tabs ] => Sets vertical tabs.");
    case TelnetOpt::Ver_Tab_Use:     
        return string("[ Ver Tab Use ] => Determines handling of vertical tabs.");
    case TelnetOpt::Lf_Use:          
        return string("[ Lf Use ] => Determines handling of line feeds.");
    case TelnetOpt::Ext_ASCII:      
        return string("[ Ext ASCII ] => Defines extended ASCII characters.");
    case TelnetOpt::Logout:              
        return string("[ Logout ] => Allows for forced log-off.");
    case TelnetOpt::Byte_Macro:      
        return string("[ Byte Macro ] => Defines byte macros.");
    case TelnetOpt::Data_Term:        
        return string("[ Data Term ] => Allows subcommands for Data Entry to be sent.");
    case TelnetOpt::SUPDUP:          
        return string("[ SUPDUP ] => Allows use of SUPDUP display protocol.");
    case TelnetOpt::SUPDUP_Outp:     
        return string("[ SUPDUP Outp ] => Allows sending of SUPDUP output.");
    case TelnetOpt::Send_Locate:     
        return string("[ Send Locate ] => Allows terminal location to be sent.");
    case TelnetOpt::Term_Type:       
        return string("[ Term Type ] => Allows exchange of terminal type information.");
    case TelnetOpt::End_Record:      
        return string("[ End Record ] => Allows use of the End of record code(0xEF).");
    case TelnetOpt::TACACS_ID:      
        return string("[ TACACS ID ] => User ID exchange used to avoid more than 1 log-in.");
    case TelnetOpt::Output_Mark:     
        return string("[ Output Mark ] => Allows banner markings to be sent on output.");
    case TelnetOpt::Term_Loc:        
        return string("[ Term Loc ] => A numeric ID used to identify terminals.");
    case TelnetOpt::Regime_3270:    
        return string("[ 3270 Regime ] => Allows emulation of 3270 family terminals.");
    case TelnetOpt::X_3_PAD:        
        return string("[ X.3 PAD ] => Allows use of X.3 protocol emulation. ");
    case TelnetOpt::Window_Size:    
        return string("[ Window Size ] => Conveys window size for emulation screen.");
    case TelnetOpt::Term_Speed:      
        return string("[ Term Speed ] => Conveys baud rate information.");
    case TelnetOpt::Remote_Flow:    
        return string("[ Remote Flow ] => Provides flow control (XON, XOFF).");
    case TelnetOpt::Linemode:        
        return string("[ Linemode ] => Provides linemode bulk character transactions.");
    case TelnetOpt::X_Display_Location:
        return string("[ X Display Location ] => Sender is willing to send the X display location in a subsequent sub-negotiation.");
        break;
    case TelnetOpt::Extended:       
        return string("[ Extended ] => options list  Extended options list.");
    default:
        return string("[ Unknown ] => Maybe new environment.");
    }
}